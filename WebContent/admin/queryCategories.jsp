...
<script type="text/javascript">
	$(function() {
		$('#dg').datagrid({
			...
			url : '${baseUrl}/category_getJsonCategories',
			loadMsg : 'Loading......',
			queryParams : {
				type : ''
			},//参数  
			...
			toolbar : [ {
				...
				handler : function() {
					 parent.$("#win").window({ //因为<div>放在了aindex.jsp中，所以这里创建窗口要先调用parent  
				            ...
				            width:350,  
				            height:150,  
				            content:'<iframe src="${baseUrl}/admin/saveCategory.jsp" frameborder="0" width="100%" height="100%"/>'  
				        });  
				}
			}, '-', {
				iconCls : 'icon-remove',
				...
				handler : function() {
					...
					if (row==null) {
			            ...
			            $.messager.confirm('删除的确认对话框', '您确定要删除此项吗？', function(r){  
			                if (r){  
			                    //发送ajax请求  
			                    $.post("${baseUrl}/category_deleteById",{id:row.id},function(result){  
			                        if(result == "true") {  
